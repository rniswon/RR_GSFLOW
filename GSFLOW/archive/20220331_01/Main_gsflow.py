...
if update_transient_model_for_smooth_running == 1:
    ...
    #     lay_idx = ghb_cell['k'].values[0]
    #     row_idx = ghb_cell['i'].values[0]
    #     col_idx = ghb_cell['j'].values[0]
    #
    #     # flag this cell and store head value
    #     ghb_arr[lay_idx, row_idx, col_idx] = 1
    #     ghb_head_arr[lay_idx, row_idx, col_idx] = ghb_cell['bhead'].values[0]
    #
    ...
    #
    #     # calculate number of ghb cells each ghb neihbor is near
    #
    #
    #
    # # flag active cells with GHB neighbors on more than one side (count the number of GHB neighbors for each grid cell in model domain) and keep track of which cells they are
    # # loop through active cells in each layer
    # #ghb_ = np.where(ghb_arr == 1)
    # # identify neighbors by adding 1 for each neighbor
    #
