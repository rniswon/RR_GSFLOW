...
def main(script_ws, model_ws, results_ws, mf_name_file_type):
    ...
    plot_folder = os.path.join(results_ws, 'plots', 'water_budget_light')
    ...
    budget_daily = pd.merge(df, streamflow_in, how='left', left_on=['subbasin', 'Date'], right_on=['subbasin', 'date'])
    ...
    for sub in subs:
        ...
        selected_vars = ['precip', 'surface_and_soil_zone_et', 'recharge', 'hortonian_flow', 'dunnian_flow', 'flow_to_lakes', 'ssres_flow', 'storage_change']
        df = df_all[df_all['variable'].isin(selected_vars)]
