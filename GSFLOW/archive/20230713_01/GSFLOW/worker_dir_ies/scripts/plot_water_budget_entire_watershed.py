...
# NOTE: the subbasin variable in this script is referring to zonebudget zones (with the entire watershed in one zone), not subbasins
...
def main(script_ws, model_ws, results_ws, mf_name_file_type, modflow_time_zero, start_date, end_date, modflow_time_zero_altformat, start_date_altformat, end_date_altformat):
    ...
    zone_budget_file = os.path.join(model_ws, "modflow", "output", "zone_budget_watershed.csv.2.csv")
    ...
    budget_subbasin_daily_file = os.path.join(results_ws, 'tables', 'budget_entire_watershed_daily.csv')

    # set file name for annual budget table
    budget_subbasin_annual_file = os.path.join(results_ws, 'tables', 'budget_entire_watershed_annual.csv')

    # set file name for monthly budget table
    budget_subbasin_monthly_file = os.path.join(results_ws, 'tables', 'budget_entire_watershed_monthly.csv')

    # set plot folder name
    plot_folder = os.path.join(results_ws, 'plots', 'water_budget_entire_watershed')
    ...
    def reformat_prms_outputs(file_name, variable_name):
        ...
        # add up over all subbasins
        df = df.groupby(['totim', 'Date'], as_index=False)[[variable_name]].sum()
        df['subbasin'] = 1
    ...
    #     upstream_sub_df = streamflow_out[streamflow_out['subbasin'].isin(upstream_sub)].copy()
    ...
    # add in potter valley flows for streamflow coming into watershed
    ...
    # add in mark west inflows for streamflow coming into watershed
    ...
    mark_west_inflows['subbasin'] = 1
    mark_west_inflows = mark_west_inflows[['date', 'streamflow_in', 'subbasin']]

    # combine into one data frame
    streamflow_in = pd.concat([potter_valley_inflows, mark_west_inflows])
    streamflow_in = streamflow_in.groupby(['date', 'subbasin'], as_index=False)[['streamflow_in']].sum()
    ...
    # place all in one subbasin and sum
    ag_div['subbasin'] = 1
