...
def main(script_ws, model_ws, results_ws, mf_name_file_type, modflow_time_zero, start_date, end_date, modflow_time_zero_altformat, start_date_altformat, end_date_altformat):
    ...
    zone_budget_file = os.path.join(model_ws, "modflow", "output", "zone_budget_gwbasin.csv.2.csv")
    ...
    # set mark west inflows file
    mark_west_inflows_file = os.path.join(model_ws, "modflow", "input", "Mark_West_inflow.dat")
    ...
    ag_direct_div_shp_file = os.path.join(script_ws, 'script_inputs', "ag_direct_div.txt")
    ag_pond_div_shp_file = os.path.join(script_ws, 'script_inputs', "ag_pond_div.txt")
    ...
    budget_subbasin_daily_file = os.path.join(results_ws, 'tables', 'budget_gwbasin_daily.csv')

    # set file name for annual budget table
    budget_subbasin_annual_file = os.path.join(results_ws, 'tables', 'budget_gwbasin_annual.csv')

    # set file name for monthly budget table
    budget_subbasin_monthly_file = os.path.join(results_ws, 'tables', 'budget_gwbasin_monthly.csv')

    # set plot folder name
    plot_folder = os.path.join(results_ws, 'plots', 'water_budget_gwbasin')
    ...
    zbout_net = ZoneBudget.read_output(zone_budget_file, net=True, dataframe=True, pivot=True, start_datetime=start_date_altformat)
    ...
    # rename
    zbout.rename(columns={"STORAGE": "STORAGE_CHANGE"}, inplace=True)



    #---- Read in ag diversion shapefile table and reformat ---------------------------------------------------------####

    # read in ag direct diversion shapefile table
    ag_direct_div_shp = pd.read_csv(ag_direct_div_shp_file)
    ag_direct_div_shp["nseg"] = ag_direct_div_shp["nseg"].astype("int")

    # read in ag pond diversion shapefile table
    ag_pond_div_shp = pd.read_csv(ag_pond_div_shp_file)
    ag_pond_div_shp["nseg"] = ag_pond_div_shp["nseg"].astype("int")
    ...
    def reformat_prms_outputs(file_name, variable_name):
        ...
        df['totim'] = df.index.values + 1 + num_days_spinup
        ...
        # add up over subbasins for groundwater basins
        df['subbasin_group'] = 0
        mask_potter = df['subbasin'].isin([2])
        mask_ukiah = df['subbasin'].isin([1,3,4,5])
        mask_sanel_mcdowell = df['subbasin'].isin([6])
        mask_alexander = df['subbasin'].isin([9,10,12])
        mask_knights = df['subbasin'].isin([11])
        mask_santa_rosa = df['subbasin'].isin([14,15,16,17])
        mask_wilson_grove_lower_rr = df['subbasin'].isin([18,19,21])
        df.loc[mask_potter, 'subbasin_group'] = 1
        df.loc[mask_ukiah, 'subbasin_group'] = 2
        df.loc[mask_sanel_mcdowell, 'subbasin_group'] = 3
        df.loc[mask_alexander, 'subbasin_group'] = 4
        df.loc[mask_knights, 'subbasin_group'] = 5
        df.loc[mask_santa_rosa, 'subbasin_group'] = 6
        df.loc[mask_wilson_grove_lower_rr, 'subbasin_group'] = 7
        df['subbasin'] = df['subbasin_group']
        df = df.groupby(['totim', 'subbasin', 'Date'], as_index=False)[[variable_name]].sum()
    ...
    def read_gage(f, modflow_time_zero="1-1-1970"):
        dic = {'date': [], 'stage': [], 'flow': [], 'month': [], 'year': []}
        m, d, y = [int(i) for i in modflow_time_zero.split("-")]
        modflow_time_zero = dt.datetime(y, m, d) - dt.timedelta(seconds=1)
        with open(f) as foo:
            for ix, line in enumerate(foo):
                if ix < 2:
                    ...
                    t = line.strip().split()
                    date = modflow_time_zero + dt.timedelta(days=float(t[0]))
                    stage = float(t[1])
                    flow = float(t[2])
    ...
    for file in sim_files:
        ...
        data = read_gage(gage_file, modflow_time_zero_altformat)
    ...
    # keep only the ouflow from the groups of subbasins for gw basins
    streamflow_out = streamflow_out[streamflow_out['subbasin'].isin([2,5,6,11,12,17,21])]
    streamflow_out['subbasin_group'] = 0
    mask_potter = streamflow_out['subbasin'] == 2
    mask_ukiah = streamflow_out['subbasin'] == 5
    mask_sanel_mcdowell = streamflow_out['subbasin'] == 6
    mask_alexander = streamflow_out['subbasin'] == 12
    mask_knights = streamflow_out['subbasin'] == 11
    mask_santa_rosa = streamflow_out['subbasin'] == 17
    mask_wilson_grove_lower_rr = streamflow_out['subbasin'] == 21
    streamflow_out.loc[mask_potter, 'subbasin_group'] = 1
    streamflow_out.loc[mask_ukiah, 'subbasin_group'] = 2
    streamflow_out.loc[mask_sanel_mcdowell, 'subbasin_group'] = 3
    streamflow_out.loc[mask_alexander, 'subbasin_group'] = 4
    streamflow_out.loc[mask_knights, 'subbasin_group'] = 5
    streamflow_out.loc[mask_santa_rosa, 'subbasin_group'] = 6
    streamflow_out.loc[mask_wilson_grove_lower_rr, 'subbasin_group'] = 7
    streamflow_out['subbasin'] = streamflow_out['subbasin_group']
    streamflow_out = streamflow_out.drop('subbasin_group', axis=1)
    ...
    # streamflow_out_all = sim_df.copy()
    # subs = [6,9,14,18]
    # upstream_sub_list = []
    # for sub in subs:
    #
    ...
    #     upstream_sub_df = streamflow_out_all[streamflow_out_all['subbasin'].isin(upstream_sub)].copy()
    ...
    # streamflow_in['subbasin_group'] = 0
    # mask_south = streamflow_in['subbasin'] == 6
    # streamflow_in.loc[mask_south, 'subbasin_group'] = 2
    # streamflow_in['subbasin'] = streamflow_in['subbasin_group']
    # streamflow_in = streamflow_in.drop('subbasin_group', axis=1)

    streamflow_out_all = sim_df.copy()
    streamflow_out_all = streamflow_out_all.rename(columns={'streamflow_out': 'streamflow_in'})
    streamflow_in = streamflow_out_all[streamflow_out_all['subbasin'].isin([2,5,6,8,11,13,17,20,22])]
    streamflow_in['subbasin_group'] = 0
    mask_ukiah = streamflow_in['subbasin'] == 2
    mask_sanel_mcdowell = streamflow_in['subbasin'] == 5
    mask_alexander = streamflow_in['subbasin'].isin([6,8,11])
    mask_santa_rosa = streamflow_in['subbasin'].isin([22,13])
    mask_wilson_grove_lower_rr = streamflow_in['subbasin'].isin([17,20])
    streamflow_in.loc[mask_ukiah, 'subbasin_group'] = 2
    streamflow_in.loc[mask_sanel_mcdowell, 'subbasin_group'] = 3
    streamflow_in.loc[mask_alexander, 'subbasin_group'] = 4
    streamflow_in.loc[mask_santa_rosa, 'subbasin_group'] = 6
    streamflow_in.loc[mask_wilson_grove_lower_rr, 'subbasin_group'] = 7
    streamflow_in['subbasin'] = streamflow_in['subbasin_group']
    streamflow_in = streamflow_in.drop('subbasin_group', axis=1)
    streamflow_in = streamflow_in.groupby(['subbasin', 'date'])['streamflow_in'].sum().reset_index()



    # add in potter valley flows for streamflow coming into zone 1
    ...
    potter_valley_inflows['subbasin'] = 1
    ...
    # add in mark west inflows for streamflow coming into zone 7
    mark_west_inflows = pd.read_csv(mark_west_inflows_file, sep='\t', header=None)   # note: used to be sep=' '
    mark_west_inflows = mark_west_inflows.rename(columns={0: 'totim', 1: 'streamflow_in', 2: 'date'})
    tmp= mark_west_inflows['date'].str.split(pat='#', expand=True)
    mark_west_inflows['date'] = tmp[[1]]
    mark_west_inflows['subbasin'] = 7
    mark_west_inflows = mark_west_inflows[['date', 'streamflow_in', 'subbasin']]

    # generate streamflow_in
    ...
    for ag_div_seg in ag_div_segs:
        ...
        mask_ag_direct_div_shp = ag_direct_div_shp['nseg'] == ag_div_seg
        subbasin_id = ag_direct_div_shp.loc[mask_ag_direct_div_shp,  'subbasin'].values[0]
    ...
    # add up over subbasins for groundwater basins
    ag_div['subbasin_group'] = 0
    mask_potter = ag_div['subbasin'].isin([2])
    mask_ukiah = ag_div['subbasin'].isin([1, 3, 4, 5])
    mask_sanel_mcdowell = ag_div['subbasin'].isin([6])
    mask_alexander = ag_div['subbasin'].isin([9, 10, 12])
    mask_knights = ag_div['subbasin'].isin([11])
    mask_santa_rosa = ag_div['subbasin'].isin([14, 15, 16, 17])
    mask_wilson_grove_lower_rr = ag_div['subbasin'].isin([18, 19, 21])
    ag_div.loc[mask_potter, 'subbasin_group'] = 1
    ag_div.loc[mask_ukiah, 'subbasin_group'] = 2
    ag_div.loc[mask_sanel_mcdowell, 'subbasin_group'] = 3
    ag_div.loc[mask_alexander, 'subbasin_group'] = 4
    ag_div.loc[mask_knights, 'subbasin_group'] = 5
    ag_div.loc[mask_santa_rosa, 'subbasin_group'] = 6
    ag_div.loc[mask_wilson_grove_lower_rr, 'subbasin_group'] = 7
    ag_div['subbasin'] = ag_div['subbasin_group']
    ag_div = ag_div.groupby(['totim', 'subbasin'], as_index=False)[['direct_div']].sum()
    ...
    for ag_pond_div_seg in ag_pond_div_segs:
        ...
        # get subbasin for this segment
        mask_ag_pond_div_shp = ag_pond_div_shp['nseg'] == ag_pond_div_seg
        subbasin_id = ag_pond_div_shp.loc[mask_ag_pond_div_shp,  'subbasin'].values[0]
    ...
    # add up over subbasins for groundwater basins
    ag_pond_div['subbasin_group'] = 0
    mask_potter = ag_pond_div['subbasin'].isin([2])
    mask_ukiah = ag_pond_div['subbasin'].isin([1, 3, 4, 5])
    mask_sanel_mcdowell = ag_pond_div['subbasin'].isin([6])
    mask_alexander = ag_pond_div['subbasin'].isin([9, 10, 12])
    mask_knights = ag_pond_div['subbasin'].isin([11])
    mask_santa_rosa = ag_pond_div['subbasin'].isin([14, 15, 16, 17])
    mask_wilson_grove_lower_rr = ag_pond_div['subbasin'].isin([18, 19, 21])
    ag_pond_div.loc[mask_potter, 'subbasin_group'] = 1
    ag_pond_div.loc[mask_ukiah, 'subbasin_group'] = 2
    ag_pond_div.loc[mask_sanel_mcdowell, 'subbasin_group'] = 3
    ag_pond_div.loc[mask_alexander, 'subbasin_group'] = 4
    ag_pond_div.loc[mask_knights, 'subbasin_group'] = 5
    ag_pond_div.loc[mask_santa_rosa, 'subbasin_group'] = 6
    ag_pond_div.loc[mask_wilson_grove_lower_rr, 'subbasin_group'] = 7
    ag_pond_div['subbasin'] = ag_pond_div['subbasin_group']
    ag_pond_div = ag_pond_div.groupby(['totim', 'subbasin'], as_index=False)[['pond_div']].sum()
    ...
    sub_names = ['Potter Valley subbasins', 'Ukiah Valley subbasins', 'Sanel and McDowell Valley subbasins', 'Alexander Valley subbasins',
                 'Knights Valley subbasins', 'Santa Rosa Valley subbasins', 'Wilson Grove Formation Highlands and Lower Russian River Valley subbasins']
    for sub, sub_name in zip(subs, sub_names):
        ...
        this_plot = df.plot(x='water_year', kind='bar', stacked=True,
                title= sub_name + ': ' + 'surface and soil zone water budget, annual sum',
        ...
        this_plot = df.plot(x='water_year', kind='bar', stacked=True,
                title= sub_name + ': ' + 'agricultural water use, annual sum',
                xlabel = "Water Year", ylabel="Volume (acre-ft)", figsize=(12, 8))
        ...
        selected_vars = ['UZF_RECHARGE', 'GW_ET', 'WELLS', 'MNW2', 'AG_WE', 'LAKE_SEEPAGE', 'STREAM_LEAKAGE',
                         'SURFACE_LEAKAGE', 'HEAD_DEP_BOUNDS', 'OTHER_ZONES', 'STORAGE_CHANGE']
        # selected_vars = ['HEAD_DEP_BOUNDS', 'LAKE_SEEPAGE', 'OTHER_ZONES', 'STREAM_LEAKAGE',
        #                  'SURFACE_LEAKAGE', 'UZF_RECHARGE', 'WELLS', 'AG_WE', 'STORAGE_CHANGE']
        df = df_all[df_all['variable'].isin(selected_vars)]
        df = pd.pivot(df, index='water_year', columns='variable', values='value').reset_index()
        this_plot = df.plot(x='water_year', kind='bar', stacked=True,
                title= sub_name + ': ' + 'groundwater budget, annual sum',
        ...
        this_plot = df.plot(x='water_year', kind='bar', stacked=True,
                title= sub_name + ': ' + 'stream network water budget, annual sum',
                xlabel="Water Year", ylabel="Volume (acre-ft)", figsize=(12, 8))
...
if __name__ == "__main__":
    ...
    main(script_ws, model_ws, results_ws, mf_name_file_type, modflow_time_zero, start_date, end_date, modflow_time_zero_altformat, start_date_altformat, end_date_altformat)
