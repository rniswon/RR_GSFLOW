...
if update_transient_model_for_smooth_running == 1:
    ...
    mf_tr = gsflow.modflow.Modflow.load(os.path.basename(mf_tr_name_file),
                                        model_ws=os.path.dirname(os.path.join(os.getcwd(), mf_tr_name_file)),
                                        load_only=["BAS6", "DIS", "NWT", "UPW", "UZF", "LAK", "WEL", "SFR"],
                                        verbose=True, forgive=False, version="mfnwt")
    ...
    # make changes to hk and vka for entire watershed
    change_factor = 0.5        # NOTE: worked when this was set to 1
    hk = hk * change_factor
    vka = vka * change_factor

    # make changes to hk and vka for layer 2
    change_factor = 10
    ...
    # zones_to_change = [490, 491, 509, 180, 181, 134]
    # #zones_to_change = [180]  # TODO: if the above gets rid of the error, try doing this to localize the K zone that needs to be changed
    #
    # # create mask
    ...
    # scale
    vks_scaling_factor = 0.05       # NOTE: worked when it was 0.1
    ...
    # # update vks: region with UZF wave errors ------------------------------------#
    #
    # # get zone names
    # K_zones_file = os.path.join(repo_ws, "MODFLOW", "init_files", "K_zone_ids_20220318.dat")
    # zones = load_txt_3d(K_zones_file)
    ...
    # mask = np.isin(zones[1,:,:], zones_to_change)
    #
    # # make changes to vks
    # change_factor = 2
    # vks[mask] = vks[mask] * change_factor
    #
    # # store changes
    # mf_tr.uzf.vks = vks
    ...
    # # update thti: region with UZF wave error ------------------------------#
    #
    # # get zone names
    # K_zones_file = os.path.join(repo_ws, "MODFLOW", "init_files", "K_zone_ids_20220318.dat")
    # zones = load_txt_3d(K_zones_file)
