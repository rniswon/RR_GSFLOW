...
model_ws = os.path.join(repo_ws, "GSFLOW", "scratch", "20230525_01")
...
for file in files_to_update:
    ...
    if file == ag_diversions_folder:
        ...
        for ag_div_file in ag_div_files:
            ...
            # update
            ag_div_df_first_row = ag_div_df.iloc[:1].copy()
            ag_div_df_first_row.iloc[0, 0] = 0  # set time step
            ag_div_df = pd.concat([ag_div_df_first_row, ag_div_df])
        ...
        # update
        df_first_row = df.iloc[:1].copy()
        df_first_row.iloc[0,0] = 0  # set time step
