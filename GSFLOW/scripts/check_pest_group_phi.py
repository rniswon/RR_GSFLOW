...
model_output_file = os.path.join(repo_ws, "GSFLOW", "worker_dir", "pest", "model_output.csv")
...
mask_pump_chg = df['obs_group'] == 'lake_stage'

# adjust weights for each group
df_test.loc[mask_drawdown, 'weight'] = df_test.loc[mask_drawdown, 'weight'] * 10.5
df_test.loc[mask_gage_flow, 'weight'] = df_test.loc[mask_gage_flow, 'weight'] * 1.6
df_test.loc[mask_heads, 'weight'] = df_test.loc[mask_heads, 'weight']
df_test.loc[mask_lake_stage, 'weight'] = df_test.loc[mask_lake_stage, 'weight'] * 1.6
df_test.loc[mask_pump_chg, 'weight'] = df_test.loc[mask_pump_chg, 'weight']
