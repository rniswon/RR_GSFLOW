...
base_mf_out_folder = OPJ(root, 'modflow', 'output')
unimp_mf_out_folder = OPJ(thisFolder, 'RR_unimpaired', 'modflow', 'output')
base_PRMS_out_folder = OPJ(root, 'PRMS', 'output')
unimp_PRMS_out_folder = OPJ(thisFolder, 'RR_unimpaired', 'PRMS', 'output')

listfile_base = OPJ(root, 'rr_tr_base.list')
if not os.path.isfile(listfile_base):
    print('{} not found'.format(listfile_base))
    getmodflow = False
listfile_unimpaired = OPJ(unimp_mf_out_folder, 'rr_tr.list')
if not os.path.isfile(listfile_unimpaired):
    print('{} not found'.format(listfile_unimpaired))
    getmodflow = False
gsflow_base = OPJ(base_PRMS_out_folder, 'gsflow.csv')
if not os.path.isfile(gsflow_base):
    print('{} not found'.format(gsflow_base))
    getgsflow = False
gsflow_unimp = OPJ(unimp_PRMS_out_folder, 'gsflow.csv')
...
if getmodflow:
    ...
    df_flux_b, df_flux_mo_b, df_flux_yr_b, df_vol_base, df_vol_mo_base, df_vol_yr_base = getbudget(listfile_base)
    ...
    df_modflow_yr = df_vol_yr_base.merge(df_vol_yr_imp, left_index=True, right_index=True, suffixes=['_base', '_unimp'])
