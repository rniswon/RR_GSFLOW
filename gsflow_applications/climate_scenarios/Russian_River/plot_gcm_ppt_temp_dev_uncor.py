...
df_hist = pd.read_csv('gcm_historical_rr.csv', header=0)
...
df_future = pd.read_csv('gcm_future_rr.csv', parse_dates=['date'])
...
plt.axvline(hist_ppt, color='gray', lw=1, ls='--')
# plt.axhline(hist_temp, color='gray', lw=1, ls='--')
...
for mod in modlist:
    for scen in scenlist:
        ...
        ax.annotate('{}-{}'.format(mod[0], scen[0]), (netppt + labeloff[n][0], nettmp + labeloff[n][1]))
        n += 1
ax.set_ylabel('departure from mean annual historical \ntemperature in degrees C', fontsize=9)
ax.set_xlabel('departure from mean annual historical precipitation in m', fontsize=9)
...
ax.set_aspect('equal')
ax.set_title('Russian River model uncorrected GCM')
#ax.legend(fontsize=7)
if savefig:
    plt.savefig('../plots/hist_future_climate_dev_rr_uncorrected.png')
