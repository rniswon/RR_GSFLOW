...
# ===============================
"""
archive_dir: is the local path for the folder that has original model files.
model_ws : is the local path for the location where the new model with forest
            protection is saved.
misc_data_dir: is a filder used to same some of the data generated by the script. 
...
if cluster_hrus_temp:
    ...
    gw_hru['ibound'] = grid.idomain[0,kij['i'], kij['j']]
    ...
    clus = gw_hru[['seg', 'cluster']].groupby('seg').agg(pd.Series.mode)
    clus = clus.reset_index()


    # drop inactive cells
    gw_hru = gw_hru[gw_hru['ibound']>0]

    if 1:

        plt.scatter(gw_hru['j'], -1 * gw_hru['i'], c=gw_hru['cluster'], s=1)
        plt.gca().set_aspect('equal')
        plt.gca().set_xlabel("Columns")
        plt.gca().set_ylabel("Rows")
...
if 1:
    plt.figure()
    plt.scatter(reach_data['j'], -1 * reach_data['i'],
                c=reach_data['flow_acc'], s=1, cmap='jet')
    plt.gca().set_aspect('equal')
    plt.gca().set_xlabel("Columns")
    plt.gca().set_ylabel("Rows")
    plt.colorbar()
...
if 1:
    plt.figure()
    plt.scatter(ag_fields_df['j'], -1 * ag_fields_df['i'],
                c=ag_fields_df['cluster'], s=1, cmap='jet')
    plt.gca().set_aspect('equal')
    plt.gca().set_xlabel("Columns")
    plt.gca().set_ylabel("Rows")
    plt.colorbar()
...
for sub in ag_fields_df['cluster'].unique():
    ...
    ax[axi, axj].plot(subbasin_temp['Date'], subbasin_temp[sub])
    irr = subbasin_temp.copy()
    irr.loc[irr['irrigate'] == 0, sub] = frost_temp
    ax[axi, axj].plot(irr['Date'], irr[sub], c = 'r')
    ax[axi, axj].set_title('Temperature zone {}'.format(sub))
    #
...
list_of_added_segments = pd.DataFrame(list_of_added_segments,
                                      columns= ['iupseg', 'mean_flow',
...
for sg in div_iupsegs:
    ...
    sg_data = pd.concat([sg_data, pd.DataFrame([new_sg_record])],
                        ignore_index=True)
    ...
    rch_data = pd.concat([rch_data, pd.DataFrame([new_rch])],
                         ignore_index=True)
