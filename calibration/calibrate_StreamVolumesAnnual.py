    ...
    for gage in gages:
        if gage in im_gages:
            if len(im_stream_volume_averages[gage]['annual_mean']) > 0:
                ...
                mon_flow = im_stream_volume_averages[gage]['month_average'][:,1]
            ...
            if len(stream_volume_averages[gage]['annual_mean']) > 0:
                ...
                mon_flow = stream_volume_averages[gage]['month_average'][:, 1]
                num_of_meas = len(np.where(np.logical_not(np.isnan(annual_flow)))[0])
                year_time_index = []
                year_month_time_index = []
                i = 0
...
def compute_month_average(stream_volume_averages):
    for gage in stream_volume_averages.keys():
        curr_gage_data = stream_volume_averages[gage]['monthly_mean']
        mon_av = []
        for i in range(1,13):
            loc = curr_gage_data[:,1] == i
            curr_mon_avaerage = np.nanmean(curr_gage_data[loc, 2])
            mon_av.append([i, curr_mon_avaerage])
        stream_volume_averages[gage]['month_average'] = np.array(mon_av)
    return stream_volume_averages
...
if __name__ == "__main__":
    ...
    sv_file = r".\Unimpaired_Stream_Volumes.xlsx"
    xl = pd.ExcelFile(sv_file)
    df_unimp = xl.parse("aggregate_flows") # this dataframe (pandas ) has the local unimpaired flow from the flints
    nwis_file = r".\sf_daily"
    gages_shape_file = r".\model_gages.shp"
    att_table = shp.get_attribute_table(gages_shape_file)
    if False:
        read_nwis_gages(nwis_file)
    else:
        gage_flow = np.load("nwis_gage_data.npy")
        gage_flow = gage_flow.all()  # [date, flow, approves] cfs , this from nwis
    ...
    stream_volume_averages = compute_month_average(stream_volume_averages)
    im_stream_volume_averages = compute_imp_stream_volume_average(df_unimp, gage_flow, start_date, end_date, att_table)
    im_stream_volume_averages = compute_month_average(im_stream_volume_averages)
    observations = dict()
    ...
    np.save('obs_stream_volume_annual.npy', observations)
    meas_file = r"str_volume_annual.meas"
    ...
    np.save("stream_volume_times_annual.npy", data_time)
    np.save("gage_order_annual.npy", gage_order)

    ### ************** Calibration Setting **************************
    ### step (1): Prepare names for parameters to calibrate
    para = []
    months = [1,2,3, 10,11,12]
    for basin in range(21):
        for mon in months:  # 20 basin
            para.append(("adj_" + str(basin)+"_"+str(mon)))

    cal_proj = calibration.Calibration()
    cal_proj.control.MParam.filename = 'str_vol_annual.tpl'
    ...
    cal_proj.control.MObser.filename = 'str_vol_annual.ins'
    cal_proj.control.MObser.obs_data['obs_val'] = obs_dict['obs_values']
    cal_proj.control.MObser.write_ins_file()

    cal_proj.control.filename = "str_vol_annual.pst"
    cal_proj.control.INFLE = "input_str_vol_annual.dat"
    cal_proj.control.OUTFLE = "output_str_vol_annual.dat"

    # model info
    cal_proj.model_python_fn = r"model_Stream_volumes_annual.py"
    cal_proj.generate_model_batch_file('runmodel_str_vol_annual.bat')
