...
def compute_ss_unimpaired_baseflow(Sim):
    ...
    try:
        sfr_out = sfr_out.get_dataframe(dropna=False)
    except:
        sfr_out = sfr_out.get_dataframe()
...
def compute_wateruse_per_subbasin(Sim):
    ...
    rr_cc = list(zip(wel['i'].values.copy(), wel['j'].values.copy()))
    ...
    rr_cc = list(zip(hru_df['HRU_ROW'].values.copy(), hru_df['HRU_COL'].values.copy()))
    hru_df['rr_cc'] = rr_cc
    ...
    rr_cc = list(zip(sfr_out['row'].values.copy(), sfr_out['column'].values.copy()))
    sfr_out['rr_cc'] = rr_cc

    ag_water_use = {}
    reach_data = pd.DataFrame(Sim.mf.sfr.reach_data)
    rr_cc = list(zip(reach_data['i'].values.copy(), reach_data['j'].values.copy()))
