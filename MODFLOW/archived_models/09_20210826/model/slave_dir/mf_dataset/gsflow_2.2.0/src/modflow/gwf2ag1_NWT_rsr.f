...
!      print *, 'qp', qp, totim, zero
      ...
      IF (ETDEMANDFLAG > 0) THEN
         IF (GSFLOW_flag == 0) THEN
           CALL demandconjunctive_uzf(kkiter)
         ELSE
           CALL demandconjunctive_prms(kkiter, agconverge)
         END IF
      END IF
      IF (TRIGGERFLAG > 0) THEN
         CALL demandtrigger_sw()
      END IF
      !
         ...
         IF (IBOUND(IC, IR, IL) > 0) THEN
            ...
            ELSEIF (Q < ZERO) THEN
               ...
               IF (ETDEMANDFLAG > 0) THEN
                  ...
                  ELSE
                     QQ = demandgw_prms(l, kkiter, agconverge)
                  END IF
               ELSEIF (TRIGGERFLAG > 0) then
                  QQ = demandtrigger_gw(Q, l)
               END IF
               IF (QQ < Q) QQ = Q
               Q = QQ
               QONLY(L) = SNGL( DONENEG*Q )
...
! ----------------------------------------------------------------------
!
      SUBROUTINE DEMANDCONJUNCTIVE_UZF(kiter)
!     ******************************************************************
...
!        IF (DEMAND(iseg) < zerod7) goto 300
        ...
        factor = set_factor(aetold, pettotal, aettotal, sup,
     +           supold, kiter)
...
!
!
      subroutine demandconjunctive_prms(kiter, agconverge)
!     ******************************************************************
!     demandconjunctive---- sums up irrigation demand using ET deficit
!     ******************************************************************
!     SPECIFICATIONS:
...
!
      double precision function demandtrigger_gw(Q, l)
!     ******************************************************************
...
!
      ...
      sup = QONLY(l)
      supold = QONLYOLD(l)
      factor = set_factor(aetold, pettotal, aettotal, sup, supold, 
     +                    kiter)
      QONLYOLD(l) = QONLY(l)
...
!
!
      double precision function demandgw_prms(l, kiter, agconverge)
!     ******************************************************************
!     demandgw---- sums up irrigation demand using ET deficit for gw
!     ******************************************************************
!     SPECIFICATIONS:
...
!
      double precision function set_factor(aetold, pettotal, 
