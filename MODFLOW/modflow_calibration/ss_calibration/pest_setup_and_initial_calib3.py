...
for obs in obsnames:
    ...
    if obs in df_weights['Obsnme'].values: # if gage or max pumping
        if obs in zero_weight_gage:
            ...
            if obs == 'pmpchg':
                w = abs(1.0/(235000/100))
            else:
                w = abs(1.0/(obsval * 1.0/100))
...
pst.parameter_data = pst.parameter_data.reindex(parnames)
pst.observation_data = pst.observation_data.reindex(obsnames)
